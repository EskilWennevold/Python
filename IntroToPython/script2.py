...
def front_back(a, b):
  if len(a)%2==0:
    firstpart = a[:int(len(a)/2)]
    secondpart = a[int(len(a)/2):]
  else:
    firstpart = a[:int(len(a)/2)+1]
    secondpart = a[int(len(a)/2)+1:]
  if len(b)%2==0:
    thirdpart = b[:int(len(b)/2)]
    fourthpart = b[int(len(b)/2):]
  else:
    thirdpart = b[:int(len(b)/2)+1]
    fourthpart = b[int(len(b)/2)+1:]
